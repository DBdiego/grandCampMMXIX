...
class RomanEmpireMap extends Component {
    ...
    createBelgiumMap(){
        ...
        screenSize = [parseFloat(d3.select('#RomanEmpireMap_' + DataHandlers.TextNeutralizer(this.props.title)).style('width' ).replace('px', '')),
                      parseFloat(d3.select('#RomanEmpireMap_' + DataHandlers.TextNeutralizer(this.props.title)).style('height').replace('px', ''))
                     ]
        if (screenSize[0] < 200){
            screenSize[0] = 200;
        };
        ...
        let empires   = topojson.feature(Empires         , Empires.objects.collection       ).features;
        ...
        let projection = d3.geoAlbers()
            ...
            .scale(Math.min.apply(null, screenSize) * 5)
            .translate([screenSize[0] * 0.40, screenSize[1] * 0.48]);
    ...
    render() {
        return (
            <svg ref = { node => this.node = node } 
                 id  = {'RomanEmpireMap_' + DataHandlers.TextNeutralizer(this.props.title)}
                 width = {'100%'}
                 height = {'100%'}
